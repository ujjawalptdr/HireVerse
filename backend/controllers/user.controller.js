...
export const register = async (req, res) => {
    try {
        ...
        //Check weather user already exist
...
export const login = async (req, res) => {
    try {
        ...
        // user exist on this email or not
...
export const updateProfile = async (req, res) => {
    try {
        ...
        //resume comes later here...  (before)
        // if (cloudResponse) {
        //     user.profile.resume = cloudResponse.secure_url // save the cloudinary url
        //     user.profile.resumeOriginalName = file?.originalname // Save the original file name
        // }

        // Upload new resume only if a file is provided...    (after)
        if (file) {
            const fileUri = getDataUri(file);
            const cloudResponse = await cloudinary.uploader.upload(fileUri.content, { resource_type: "auto" });

            if (cloudResponse) {
                user.profile.resume = cloudResponse.secure_url; // Save the new Cloudinary URL
                user.profile.resumeOriginalName = file.originalname; // Save the original file name
            }
        }
